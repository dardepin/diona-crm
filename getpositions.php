...
function getpositions($db, $page, $lim, $name)
{
    ...
    if($name == '') $sel = 'SELECT enumlabel FROM pg_enum WHERE enumtypid = (SELECT oid FROM pg_type WHERE typname = \'positions\')';
    else $sel = 'SELECT enumlabel FROM pg_enum WHERE enumlabel LIKE \'%' . $name . '%\' AND enumtypid = (SELECT oid FROM pg_type WHERE typname = \'positions\')';
